...
function parseBlockFormat(text) {
  ...
  const blockRegex = /Type:\s*(?:Limit\s*|Market\s*|Fractional\s*|Stop\s*(?:Limit\s*)?)?(?<type>Buy|Sell)\s+Symbol:\s*(?<ticker>[A-Z0-9.\-]+)\s+Shares:\s*(?<shares>[\d,.]+)\s+(?:Average\s+price|Price):\s*\$?(?<price>[\d,.]+)/gi
